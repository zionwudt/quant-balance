...
def test_render_demo_page_shows_friendly_validation_error_for_invalid_ma_combo() -> None:
    html = render_demo_page(form_data={"input_mode": "example", "symbol": "600519.SH", "initial_cash": "100000", "short_window": "20", "long_window": "10"})
...
def test_render_demo_page_shows_short_sample_warning_context_export_and_equity_curve() -> None:
    html = render_demo_page(form_data={"input_mode": "example", "symbol": "600519.SH", "initial_cash": "100000", "short_window": "5", "long_window": "10"})
...
def test_create_app_handles_health_and_multipart_file_upload_flow(tmp_path: Path) -> None:
    ...
    health_response = app({"REQUEST_METHOD": "GET", "PATH_INFO": "/health", "wsgi.input": BytesIO(b""), "CONTENT_LENGTH": "0"}, start_response)
    ...
    page_response = app({"REQUEST_METHOD": "POST", "PATH_INFO": "/demo", "CONTENT_TYPE": f"multipart/form-data; boundary={boundary}", "wsgi.input": BytesIO(multipart_body), "CONTENT_LENGTH": str(len(multipart_body))}, start_response)
